(function initializeAtlasCore(root, factory) {
  ...
  function normalizeCountyToken(name) {
    ...
    const jurisdiction = scopedMatch ? scopedMatch[1].trim() : normalized;
    const scopedSuffix = scopedMatch ? scopedMatch[2].trim() : '';
    let canonical = jurisdiction;

    if (
      /^CITY OF (ST\.?\s+LOUIS|SAINT\s+LOUIS)$/i.test(jurisdiction)
      || /^(ST\.?\s+LOUIS|SAINT\s+LOUIS)\s+(CITY|INDEPENDENT CITY)$/i.test(jurisdiction)
    ) {
